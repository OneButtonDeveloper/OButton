...
(function() {
  ...
  if (!window.oneButton) {
    oneButton = {};
    oneButton.extension = (function() {
      var extensions;
      extensions = {};
      return function(name) {
        if (name) {
          return extensions[name] != null ? extensions[name] : extensions[name] = {};
        } else {
          return extensions;
        }
      };
  ...
  (function(GithubExtension) {
    var isFirstTimeClick;
    isFirstTimeClick = true;
    return GithubExtension.onClick = function() {
      var Commits, Day, url;
      url = window.location.toString();
      if (url.indexOf('github') > 0 && url.indexOf('commits') > 0 && isFirstTimeClick) {
  ...
  (function(LorenIpsum) {
    ...
    return shortcut.add("Alt+F1", function() {
      var el, name1;
      if (el = document.activeElement) {
        return typeof handlers[name1 = el.tagName.toLowerCase()] === "function" ? handlers[name1]($(el)) : void 0;
      }
    });
  })(window.oneButton.extension("LorenIpsum"));

  kango.addMessageListener('click', function(event) {
    var extension, name, ref;
    oneButton = window.oneButton;
    if (!oneButton) {
      console.log('oneButton not initialized');
      return;
    }
    ref = oneButton.extension();
    for (name in ref) {
      extension = ref[name];
